...
def log_error (errorstring):
	...
	errorstring = errorstring.replace(eol_char_tx, "")				# if complete send commands are pasted into error string, cr/lf char is removed
	errorstring = errorstring.replace(eol_char_rx, "")				# if complete received lines are pasted into error string, cr/lf char is removed
...
def sercom (sendstring):
		...
		if ser.inWaiting() > 0:										# received character waiting in uart buffer
			...
			if s == eol_char_rx:									# end of line character received?
				...
				if len(input_line) >= max_buffer_length:			# maximum buffer length reached?
					saveline = True
			if saveline == True:
				pos = input_line.find(":") 							# find first occurrence of character ":". if it is at beginning of received line, ..
				if pos == 0:
	...
	if (timeouterror == True):
		...
		if pos != -1:												# empty 
			payload, checksum_received = returnstring.split("#")	# part before "#" is payload, part after "#" is checksum
